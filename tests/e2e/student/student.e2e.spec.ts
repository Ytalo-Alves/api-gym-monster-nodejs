...
describe("Student - e2e Test", () => {
  ...
  it("Should be able to get a student by id", async () => {
    const listResponse = await request(app.server).get("/students");
    const studentId = listResponse.body[0].id;

    const response = await request(app.server).get(`/students/${studentId}`);
  ...
  it("Should be able to delete a student", async () => {
    ...
    const response = await request(app.server).delete(`/students/${studentId}`);

    expect(response.statusCode).toBe(204);

    const getResponse = await request(app.server).get(`/students/${studentId}`);
